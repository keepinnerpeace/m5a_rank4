...
class M5aMain(object):
    ...
    def generate_grid_base(self, train_df, prices_df, calendar_df):
        """
        处理train_df数据，首先对数据进行转换，将“天”转换为新的一列数据，
        然后再把验证数据添加上，最后再基于prices_df数据计算商品最新开始售卖的周，再基于
        calendar_df数据匹配上d所属哪一个周，最后再将每个商品最开始售卖的周减去所有
        商品开始售卖周的最小值，然后将各个商品最开始的周减去所有商品开始售卖周的最小值，
        作为release列
        :param train_df:
        :param prices_df:
        :param calendar_df:
    ...
    def generate_grid_price(self, prices_df, calendar_df):
        ...
        # prices_df['sell_price_cent'] = prices_df['sell_price'].apply(lambda x:math.modf(x)[0])
