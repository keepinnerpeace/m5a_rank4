...
class WRMSSEEvaluator(object):
    ...
    def score(self, valid_preds: Union[pd.DataFrame, np.ndarray]) -> float:
        assert self.valid_df[self.valid_target_columns].shape == valid_preds.shape # 检查 valid_preds 的形状是否正确

        if isinstance(valid_preds, np.ndarray):
            valid_preds = pd.DataFrame(valid_preds, columns=self.valid_target_columns)

        valid_preds = pd.concat([self.valid_df[self.id_columns], valid_preds], axis=1, sort=False) # 将 valid_preds 与 id_columns 连接

        all_scores = []
        for i, group_id in enumerate(self.group_ids):
            valid_preds_grp = valid_preds.groupby(group_id)[self.valid_target_columns].sum() # 按 group_id 分组，计算预测值的总和
            setattr(self, f'lv{i + 1}_valid_preds', valid_preds_grp)

            lv_scores = self.rmsse(valid_preds_grp, i + 1) # 计算 RMSSE
            setattr(self, f'lv{i + 1}_scores', lv_scores)

            weight = getattr(self, f'lv{i + 1}_weight')
            lv_scores = pd.concat([weight, lv_scores], axis=1, sort=False).prod(axis=1) # 计算 RMSSE 与权重的乘积

            all_scores.append(lv_scores[~lv_scores.isin([np.inf])].sum()) # 将所有 RMSSE 与权重的乘积相加
        self.all_scores = all_scores

        return np.mean(all_scores) # 返回所有 RMSSE 与权重的乘积的平均值
